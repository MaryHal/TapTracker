...
compiler = args.cxx
include = ['-isystem./ext/uthash/include', '-isystem./ext/stb', '-isystem./ext/incbin', '-isystem./ext/glad_gen/include']
depends = []
libdirs = []
ldflags = ['-lGL', '-lglfw', '-ldl', '-lm', '-lrt']
...
testobjs = []
srcfiles = files_from('src/', '**.c') + ['ext/glad_gen/src/glad.c']
for f in srcfiles:
    obj = object_file(f)
    testobjs.append(obj)

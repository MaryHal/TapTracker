* TapTracker  [[https://travis-ci.org/sanford1/TapTracker][https://travis-ci.org/sanford1/TapTracker.svg?branch=master]] :TOC_5_gh:
   ...
   - [[#to-do-list][To-do list]]
     - [[#document-tap-states-][Document TAP states ]]
     - [[#implement-resizable-layouts-][Implement resizable layouts ]]
     - [[#rework-opengl-code-][Rework OpenGL code ]]
     - [[#player-2-support-][Player 2 support ]]
     - [[#diagram-generation-][Diagram Generation ]]
     - [[#mame-patch-for-windows][MAME patch for Windows]]
     - [[#more-error-logging-][More error logging ]]
     - [[#config-files-json-][Config files (json?) ]]
   - [[#appendix][Appendix]]
     - [[#tap-tgm2p][TAP (tgm2p)]]
...
** To-do list
*** DONE Document TAP states 
[Mostly done] I've reverse engineered most of the TGM2+ states (read: I've figured out what they mean), but there may be edge cases that still confuse TapTracker.
*** DONE Implement resizable layouts 
[Mostly done] TapTracker only supports a single window size and layout right now, and it's difficult to modify.
*** TODO Rework OpenGL code 
...
*** DONE Diagram Generation 
Create a [[http://fumen.zui.jp/][Fumen]] diagram during play. A old (deprecated) prototype (written in Python) can be found in the =autofumen= directory. This functionality is now built into shmupmametgm.
*** DONE MAME patch for Windows
No longer necessary, shmupmametgm fork supports Windows. But TapTracker does not!
...
*** TODO Config files (json?) 
For configurable layouts and to possibly replace the command line arguments.
